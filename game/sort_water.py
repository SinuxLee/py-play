...
capacity = 4  # 每个瓶子的容量
bottle_amount = 30  # 瓶子总数
empty_bottle = 2  # 两个空瓶

class WaterSortSolver:
    def __init__(self, bottles, max_capacity=4):
    ...
    def get_top_color_count(self, bottle):
        """获取瓶子顶部相同颜色的数量"""
        if not bottle:
            return None, 0
        color = bottle[-1]
    ...
    def is_bottle_complete(self, bottle):
        """检查单个瓶子是否已完成"""
        return len(bottle) == self.max_capacity and len(set(bottle)) == 1
    
    def is_bottle_single_color(self, bottle):
        """检查瓶子是否只有一种颜色（但可能未满）"""
        return len(bottle) > 0 and len(set(bottle)) == 1
    
    def count_complete_bottles(self, bottles):
        """统计已完成的瓶子数"""
        return sum(1 for b in bottles if self.is_bottle_complete(b))
    
    def is_valid_pour(self, from_bottle, to_bottle):
        """检查倒水是否合法"""
    ...
    def is_useful_move(self, state, from_idx, to_idx):
        """判断这次移动是否有意义 - 强化版剪枝"""
    ...
    def pour_water(self, bottles, from_idx, to_idx):
        """执行倒水操作"""
    ...
    def state_to_tuple(self, bottles):
        """将状态转换为元组用于哈希"""
        return tuple(tuple(b) for b in bottles)
    
    def get_heuristic(self, bottles):
        """改进的启发式函数"""
        h = 0
        
        # 统计当前每种颜色的分布
        color_bottles = {}  # 每种颜色在哪些瓶子里
    ...
    def solve(self, max_steps=500000, time_limit=300):
        ...
        print(f"开始求解... 初始启发值: {initial_h:.2f}")
        print(f"总瓶子数: {len(self.initial_state)}, 空瓶数: {sum(1 for b in self.initial_state if not b)}")
        print(f"颜色种类: {len(self.color_count)}")
        print()
        ...
        while heap:
            ...
            if elapsed > time_limit:
                print(f"\n超时 ({time_limit}秒)，停止搜索")
                print(f"已搜索 {steps} 步, 最大队列: {max_queue_size}")
                return None
            
            # 检查步数限制
            if steps > max_steps:
                print(f"\n搜索步数过多 (>{max_steps})，停止搜索")
                print(f"最大队列大小: {max_queue_size}")
                return None
            
            # 定期报告进度
            if steps % 20000 == 0:
                print(f"进度: {steps:>6} 步 | 队列: {len(heap):>5} | "
                      f"最佳: {best_complete} 个完成 | 耗时: {elapsed:>6.2f}秒")
            
            f_value, cost, current_state, path = heappop(heap)
...
solver = WaterSortSolver(bottles)
solution1 = solver.solve(max_steps=1000, time_limit=3)
if solution1:
    solver.print_solution(solution1, verbose=True)
    print(f"✓ 成功解决！总共 {len(solution1)} 步\n")
else:
    print("✗ 未找到解决方案\n")